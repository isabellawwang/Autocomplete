...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		int firstPossible = firstIndexOf(myTerms, new Term(prefix, 0), new Term.PrefixOrder(prefix.length()));
		int lastPossible = lastIndexOf(myTerms, new Term(prefix,0), new Term.PrefixOrder(prefix.length()));
		if (k==0 || firstPossible < 0 || lastPossible < 0)
		{
		...
		for (int i = firstPossible; i<=lastPossible; i++) {
			Term toAdd = myTerms[i];
			if (!toAdd.getWord().startsWith(prefix))
				continue;
			if (pq.size() < k) {
				pq.add(toAdd);
			}
			else if (pq.peek().getWeight() < toAdd.getWeight()) {
				pq.remove();
				pq.add(toAdd);
			}
		}
	...
	/**
	 * This method serves as a helper method for the constructor and helps build a BinarySearchAutocomplete.
	 * In this case, it helps initialize and fill the array of terms myTerms.
	 *
	 * @param terms
